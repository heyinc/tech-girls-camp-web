...
## Claude Code で作りたいものを作ってみる

ここまでで HTML/CSS/JavaScript/Web API の基本に触れてきました。
ここからは Claude Code に「こういうものを作りたい」と伝えて、新しいページを作ってもらうことに挑戦してみましょう。

たとえば以下のような依頼ができます。

```
omikuji.html を作って。ボタンを押すと「大吉」「中吉」「小吉」「凶」などがランダムに表示されるおみくじページを作って。
```

```
todo.html を作って。タスクを追加・削除できるTo-Doリストにして。
ページを閉じても入力したタスクが消えないようにして。
```

```
cat.html を作って。猫の写真をランダム表示するページを作って。
API は https://api.thecatapi.com/v1/images/search が使えます。
```

Claude Code は新しいファイルを作ったり、既存のファイルを書き換えたりしてくれます。
ファイルができたらブラウザで開いて動作を確認してみましょう。
うまく動かなければ、画面の見た目やエラー内容をそのまま Claude Code に伝えれば直してくれます。

以下のセクションでは、Claude Code に依頼するときのヒントとして、いろいろな技術の例を紹介します。
「これを使ったページを作って」とそのまま依頼してもよいですし、コードを読んで学ぶ材料として読むこともできます。
